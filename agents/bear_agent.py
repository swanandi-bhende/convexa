...
def _format_market_data_for_llm(market_snapshot: MarketSnapshot) -> str:
    """Create a natural-language market summary that improves LLM consistency."""
    return (
        f"Token pair: {market_snapshot.token_pair}. "
        f"Current price: {market_snapshot.current_price:.6f}. "
        f"24h baseline price: {market_snapshot.baseline_price_24h:.6f}. "
        f"24h price change: {market_snapshot.price_change_24h_pct:.2f}%. "
        f"Volume delta: {market_snapshot.volume_delta_24h_pct:.2f}% "
        f"(last 24h: ${market_snapshot.volume_last_24h_usd:,.2f}, prior 24h: ${market_snapshot.volume_prev_24h_usd:,.2f}). "
        f"Large wallet outflows in last 2h: {market_snapshot.wallet_outflow_count_2h} transactions. "
        f"Pool address: {market_snapshot.pool_address}."
    )
...
def generate_bear_argument(market_snapshot: MarketSnapshot, round_number: int) -> dict[str, Any]:
    ...
    market_summary = _format_market_data_for_llm(market_snapshot)
